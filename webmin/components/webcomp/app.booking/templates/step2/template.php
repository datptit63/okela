...
//                   Cdebug::dump($GLOBALS["USER"]->GetFullName);
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 padding_top">
                                <div class="has-error">
                                    ...
                                    <input type="text" placeholder="Bạn hãy nhập họ và tên" class="form-control" name="fullname" id="fullname" value="<?=$GLOBALS["USER"]->GetFullName() ?>"> </div>
                            </div>
                            <div class="col-md-4 padding_top">
                                <div class="has-error">
                                    <label class="control-label" for="phone">Điện thoại liên hệ*:</label>
                                    <input type="text" placeholder="Nhập số điện thoại" class="form-control" name="phone" id="phone" value="<?=$arrUser['PERSONAL_PHONE']?>"> </div>
                            </div>
                            <div class="col-md-4 padding_top">
                                <label for="email">Email:</label>
                                <input class="form-control" name="email" id="email" value="<?=$GLOBALS["USER"]->GetEmail()?>"> </div>
                            ...
                            <div class="col-md-12 padding_top has-error" hidden>
        ...
        $.ajax({
            ...
            success: function(data){
                if(data.success && data.Booking_id > 0){
                    window.location.href = "/app/app/thanh-cong/?booking_id="+data.Booking_id
                }else{
                    $("#box_error").html(data.error_text)
                }

                //bo quan man thanh toan
//                if(data.success){
